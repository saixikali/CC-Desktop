...
- **台账**：每次补丁完成后在 `CHANGELOG.md` 追加一条（日期、成员、摘要、改前/改后 sha256）。这是"回滚到哪一步"的唯一可靠依据。
...
若「当前线上包」的哈希与磁盘实测不一致，说明台账漏记或记错 —— **先补台账（跑 sync-ledger），别改这张表**。历史教训：2026-09-26 有 6 个连续补丁只写了条目、忘了更新锚点块，直到 `check-ledger` 亮红才发现。
